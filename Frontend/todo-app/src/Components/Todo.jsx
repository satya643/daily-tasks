...
export default function Todo ( ){
    ...
    return (
        <>
        <Text textAlign='center'  mt='60px'>{task.length === 0 && "There are no pending tasks, please add one."}</Text>
        <SimpleGrid columns={[1,1,1,1]} w={{base : '95%'}} m='auto' rowGap='20px' marginBottom='20px' mt='2%'>
            {task.length > 0 && task.map((elem)=>{
                return <Box key={elem._id} border={elem.status ? '2px solid green' : '2px solid #3379c1'} width={{base :'95%'}} m='auto' padding={{base :'20px'}} lineHeight={{base :'30px'}} mt={{base : '10px'}} borderRadius='10px'>
                   <Flex justifyContent={'space-between'}>
                    ...
                    <Flex justifyContent={'space-between'}>
                        ...
                        <Text fontWeight='500' fontSize={{base : '10px', md : '10px', lg : '12px'}} color='gray'>{"Due On : "+ elem.date}</Text>
                    </Flex>
                    <Flex alignItems='center' gap={'40px'}>
                         <Text color ={elem.status ? 'green' : 'red'}>{elem.status ? "Completed" : "In Progress"}</Text>
